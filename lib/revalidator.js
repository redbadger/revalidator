  ...
  validate.defaults = {
      ...
      /**
      * <p>
      * {@link #locale} for supporting localized error messages.
      * Expects {@link #schema.messages[propertyName]} to be of type {Object}. For example: { messages: { required: { en: 'English', de: 'German' } }
      * If a locale cannot be found, the first error message with a value is returned.
  ...
  function validateObject(object, schema, options, errors) {
    var props, allProps = Object.keys(object),
        visitedProps = [],
        locale = options ? options.locale : validate.defaults.locale;
    ...
    if (schema.additionalProperties) {
      ...
      if (schema.additionalProperties === false && unvisitedProps.length > 0) {
        for (i = 0, l = unvisitedProps.length; i < l; i++) {
          error("additionalProperties", unvisitedProps[i], object[unvisitedProps[i]], false, errors, locale);
        }
      }
  ...
  function validateProperty(object, value, property, schema, options, errors) {
    ...
    function constrain(name, value, assert) {
      if (schema[name] !== undefined && !assert(value, schema[name])) {
        error(name, property, value, schema, errors, locale);
      }
    }

    if (value === undefined) {
      if (schema.required && schema.type !== 'any') {
        return error('required', property, undefined, schema, errors, locale);
      } else {
        return;
      }
    }
    ...
    if (schema.format && options.validateFormats) {
      ...
      if (!spec) {
        if (options.validateFormatsStrict) {
          return error('format', property, value, schema, errors, locale);
        }
      }
      else {
        if (!spec.test(value)) {
    ...
    if (schema['enum'] && schema['enum'].indexOf(value) === -1) {
      error('enum', property, value, schema, errors, locale);
    }

    // Dependencies (see 5.8)
    if (typeof schema.dependencies === 'string' &&
        object[schema.dependencies] === undefined) {
      error('dependencies', property, null, schema, errors, locale);
    }
    ...
    checkType(value, schema.type, function(err, type) {
      if (err) return error('type', property, typeof value, schema, errors, locale);
  ...
  function getLocalizedMessage(messages, currentLocale) {
    if (typeof messages === 'object')
    {
      // Check to see if the messages has a localized message
      if (currentLocale && messages.hasOwnProperty(currentLocale)) {
        return messages[currentLocale];
      } else {
        // if no locale can be found, just get the first non-null value
  ...
  function error(attribute, property, actual, schema, errors, locale) {
    ...
    message = getLocalizedMessage(message, locale);
